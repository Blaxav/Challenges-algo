...
def getOutput(vals : list, input=0):
    vals = mylist(vals)
    i = 0
    while 1:
        opcode = (vals[i] % 100,
        ...
        # 1 Parameter
            ...
            a = vals[i+1] if not opcode[1] else i+1
            if opcode[0] == 3: # Input
                vals[a] = input
        ...
        # 2 Parameters
            ...
            a = vals[i+1] if not opcode[1] else i+1
            b = vals[i+2] if not opcode[2] else i+2
        ...
        # 3 Parameters
            ...
            a = vals[i+1] if not opcode[1] else i+1
            b = vals[i+2] if not opcode[2] else i+2
            c = vals[i+3] if not opcode[3] else i+3
...
def part1(vals: list):
    memory = vals.copy()
    return getOutput(memory,input=1)

def part2(vals: list):
    memory = vals.copy()
    return getOutput(memory,input=5)
