...
class Node :
    ...
    def set_info(self, father, index, key, dist, portes) :
        #self.index = index
        self.longueur = father.longueur + dist[key][father.chemin[-1]]
        self.chemin = father.chemin.copy()
        self.chemin.append(key)

        # on update reachable selon la nouvelle cle ajoutee au chemin
        self.reachable = [i for i in father.reachable if i != key]
        for z in sorted(father.notreachable, key=lambda dst: dist[key]) :
            # 1er niveau, distance
            good = True
            for v in father.reachable :
                print("NR : ", z, " R : ", v, dist[key][v] + dist[v][z], dist[key][z])
                if v!=z and dist[key][v] + dist[v][z] == dist[key][z] :
                    good = False
            
            # second niveau, portes
            if good and not [k for k in portes[z] if k not in self.chemin] :
                self.reachable.append(z)
            else :
                self.notreachable.append(z)
        

        print(self.chemin, self.reachable, self.notreachable, self.longueur)
        print()

    def reachable_keys_init(self, keys, doors, dist) :
        result = deque()
        ...
        for v in result :
            ...
            notgood = False
            for z in result :
                if v!=z and dist[init][z] + dist[z][v] == dist[init][v] :
                    notgood = True
            if not notgood :
                reach.append(v)
    ...
    def heuristique(self, tree) :
        ...
        #print("Noeuds manquant : ", liste_noeuds)
...
class Tree :
    def __init__(self, keys, dist_mat, portes, UB) :
        ...
        self.nbr_nodes = 1
        first_node = Node()
        first_node.reachable_keys_init(self.keys, self.doors, self.dist)
        first_node.notreachable = [i for i in self.keys if i not in first_node.chemin and i not in first_node.reachable]
        print(first_node.chemin, first_node.reachable, first_node.notreachable)
        #exit()
    ...
    def expand_tree(self) :
        ...
        #print(current_node.chemin, " L : ", current_node.longueur)
        ...
        if len(current_node.chemin) == len(self.keys) :
            ...
            for v in current_node.reachable :    
...
def run_tree_search(instance) :
    ...
    ################################################
    # Modification sutrutre de données
    # @ -> 0, a -> 1, etc...
    ################################################
    newkeys = [i for i in range(len(keys))]
    id_to_key = [ascii_lowercase[i] for i in range(len(keys)-1) ]
    id_to_key.insert(0, '@')
    ...
    for k in id_to_key :
        newportes.append([])
        for p in portes[k] : 
            newportes[-1].append( id_to_key.index(p) )

    #for k in range(len(keys)) :
    #    print(id_to_key[k], newportes[k])
    ...
    for k in id_to_key :
        ...
        for p in id_to_key :
            if p == k :
                ...
                newdist[-1].append(dist_mat[(k,p)])

    #for k in range(len(keys)) :
        #for p in range(len(keys)) :
        #print(id_to_key[k], newdist[k])
    ...
    tree = Tree(newkeys, newdist, newportes, nearest_neighbour_cost)
    

    for t in range(len(id_to_key)) :
        print(t, id_to_key[t])
    print()
